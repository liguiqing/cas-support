...
public class EncryptionUtils {
	...
	public static byte[] getHmacSHA1(String data, String key) {
		...
		try {
			tempResult = sha1.getDigestOfBytes(join(ipadArray, data
					.getBytes("utf-8")));
		} catch (UnsupportedEncodingException e) {
			tempResult = sha1
					.getDigestOfBytes(join(ipadArray, data.getBytes()));
		}
	...
	public static void main(String[] args) {
		...
		String timestamp = "1382446117182";
		...
		String appid = "D855A1C2B67E0D002A455A07EDAAA921";
		String appkey = "B73A61321F54C9144A1AADA8F24E18EE";
		long timestamp = System.currentTimeMillis(); 
		System.out.println(timestamp);
		String paramValues = appid + appkey + timestamp;
		byte[] hmacSHA = EncryptionUtils.getHmacSHA1(paramValues, appkey);
		
		System.out.println(EncryptionUtils.bytesToHexString(hmacSHA));
		/**
		 * String str1="/aam/rest/sdfsd"; String regx="/aam/"; Pattern pattern =
		 * Pattern.compile(regx); Matcher matcher = pattern.matcher(str1);
		 * if(matcher.find()){ System.out.println("matcher1"); }else{
